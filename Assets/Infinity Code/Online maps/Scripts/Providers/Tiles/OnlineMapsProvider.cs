...
public class OnlineMapsProvider
{
    ...
    public class MapType
    {
        ...
        {
            bool useLabels = hasLabels ? OnlineMaps.instance.labels : labelsEnabled;
            if (useLabels)
            {
                if (!string.IsNullOrEmpty(_urlWithLabels)) return GetURL(zoom, x, y, _urlWithLabels, true);
                if (!string.IsNullOrEmpty(provider.url)) return GetURL(zoom, x, y, provider.url, true);
                return GetURL(zoom, x, y, _urlWithoutLabels, false);
            }

            if (!string.IsNullOrEmpty(_urlWithoutLabels)) return GetURL(zoom, x, y, _urlWithoutLabels, false);
            if (!string.IsNullOrEmpty(provider.url)) return GetURL(zoom, x, y, provider.url, false);
            return GetURL(zoom, x, y, _urlWithLabels, true);
        }
        ...
        private string GetURL(int zoom, int x, int y, string url, bool labels)
        {
            url = Regex.Replace(url, @"{\w+}", delegate (Match match)
            {
                string v = match.Value.ToLower().Trim('{', '}');
                ...
                if (v == "lng") return OnlineMaps.instance.language;
                if (v == "ext") return ext;
                if (v == "prop") return labels ? propWithLabels : propWithoutLabels;
                if (v == "prop2") return prop2;
                if (v == "variant") return labels ? variantWithLabels : variantWithoutLabels;
